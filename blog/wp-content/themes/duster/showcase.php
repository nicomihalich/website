			...
			<div id="content" role="main">
				...
				<?php endif; ?>
					...
					<?php
						...
						if ( has_post_thumbnail() ) {
							if ( $image[1] >= HEADER_IMAGE_WIDTH ) { ?>
								<a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s', 'duster' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"> <?php the_post_thumbnail( 'large-feature' ); ?></a>
							<?php } else { ?>
								<a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s', 'duster' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php the_post_thumbnail( 'small-feature' ); ?></a>
							<?php }
						}					
					?>					
				...
				<div class="widget-area" role="complementary">
					<?php if ( ! dynamic_sidebar( 'sidebar-2' ) ) : ?>
						...
						<?php
						the_widget( 'Duster_Ephemera_Widget', '', 'before_title=<h1 class="widget-title">&after_title=</h1>' );
						?>						
